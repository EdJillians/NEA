...
def populate_requirement(conn): # this function populates the requirement table with some sample data for mathematics and physics courses
    with conn.cursor() as cur:
        grades = ['A*', 'A', 'B', 'C', 'D']
        ...
        query = """SELECT course_id, tariff_001, tariff_048, tariff_064, tariff_080, tariff_096, tariff_112, tariff_128, tariff_144, tariff_160, tariff_176, tariff_192, tariff_208, tariff_224, tariff_240
        FROM course 
        WHERE course_name ILIKE '%physics%';"""
        cur.execute(query)
        courses = cur.fetchall()
        for course in courses:
            course_id = course[0]
            tariffs = [t for t in course[1:] if t is not None]
            if not tariffs:
                continue
            max_tariff = max(tariffs)
            max_tariff_index = course[1:].index(max_tariff)
